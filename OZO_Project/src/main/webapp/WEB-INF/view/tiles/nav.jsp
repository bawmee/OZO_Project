					...
					<div class="span12" style="margin-top:10px">						
							...
							<strong><i class="icon-search"></i>유형으로 찾기</strong>
								<table class="table table-bordered collapse transtable" >
						        	<tbody>
						        		<c:forTokens var="item" items="다목적홀,세미나,엠티,스튜디오,연습실,공연장,작업실,스터디룸,회의실,코워킹스페이스,파티룸,공유주방" delims="," varStatus="status">
							...
							<c:choose>
								<c:when test="${!empty memberSession.member}">
									<ul>
										...
										<c:if test="${memberSession.event == 0 and memberSession.member.memberType ne 'provider'}">                           
                              				<li><a href="<c:url value="/event/check" />"><font color="red">이</font><font color="blue">벤</font><font color="green">트</font></a></li>
                              			</c:if>
									</ul>
								</c:when>
								<c:otherwise>
