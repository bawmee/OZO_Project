    ...
    <section id="content">
      ...
      <div class="container">
        <div class="row">
            ...
            <!-- 내이름 숨기기 --> <span id="hostname" style="color:white">${reserve.space.member.name}</span>
            <table class="reservetable">
            ...
            <c:if test="${reserve.state eq 1}"> <!-- 후기 작성 완료 상태-->
            	<table class="reservetable">	
            	<thead>	
            		<tr> 
            			<th>작성한 후기  </th> 
            ...
            <aside class="right-sidebar" id="banner">
              ...
              <div class="widget">
                ...
                <ul class="recent">
                  ...
                  <li>
                    <b id="nowstate"><i class="icon-bell"></i>
                    <c:choose>
									...
									<c:otherwise>
										<span style="color:crimson">예약 반려</span></b>
									</c:otherwise>
					</c:choose>
                  ...
                  <li>
                    <b><i class="icon-calendar-empty icon-2"></i>예약 날짜</b>
                    <p>
                  ...
                  <li>
                    <b><i class="icon-user"></i></i>예약 인원</b>
                    <p>
                      ${reserve.peoplenum}
                    </p>
                  </li>
              ...
              <div id ="statebutton" class="span4">
          	 <c:choose>
					...
					<c:when test="${reserve.state eq 0}"> <!--결제 완료 상태 -->
					...
					<fmt:parseNumber value="${reserve.create_date.time/ (24*60*60*1000)}" integerOnly="true" var="createDate"/> 
						<c:if test="${memberSession.member.memId eq reserve.space.member.memId}"> <!-- 나느 호스ㅌ   -->
        						...
        						<a class="btn btn-blue-outline btn-large bigbutton" onclick="Deny()" >예약 반려</a>        															
...
<img id="buttonreplacement" src="<c:url value='/resources/img/ozo.gif'/>" alt="loading..." style="display: none; position:absolute; width:200px; height:200px; top: 30%; left:40%; margin: 0px; ">   
	...
	function Permit(){
		...
		$.ajax({
			...
			success: function(result){
				document.getElementById("nowstate").innerHTML = "<i class='icon-bell'></i>승인완료</i>";
	...
	function Deny(){
		 ...
		 if (confirm("예약을 정말 반려하시겠습니까?") == true){
			 var reason = prompt("예약 반려 사유를 입력해주세요"+"");
		 }else{   
		     return false;
		 }
		...
		var reserve_id = document.getElementById("reserveId").innerHTML;
		console.log("++" + reserve_id);
		var param = JSON.stringify({"reserve_id" : Number(reserve_id), "reason" : reason, "state" : -2});
		
		  document.body.style.opacity = "0.5"; // to display
		...
		$.ajax({
			...
			data : param,
			success: function(result){
				document.getElementById("nowstate").innerHTML = "<i class='icon-bell'></i>예약 반려<i>";
				document.getElementById("nowstate").style.color='crimson';
				document.getElementById("statebutton").style.display = 'none';
	...
	function Cancle(){
		...
		if (confirm("예약을 정말 취소하시겠습니까?") == true){    
		 }else{   
		    return;
		 }
		...
		$.ajax({
			...
			data : param,
			success: function(result){
				document.getElementById("nowstate").innerHTML = "<i class='icon-bell'></i>취소완료</i>";
				document.getElementById("statebutton").style.display = 'none';
